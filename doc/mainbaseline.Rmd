...
```{r wkdir, eval=FALSE}
set.seed(2018)
# setwd("/Users/somno/Documents/GitHub/Fall2018-Proj3-Sec2-grp11") 
...
```{r train_label}
extra_label <- read.csv("doc/label.csv", colClasses=c("NULL", NA, NA))
head(extra_label)
```
...
save(dat_train, file="output/finalfeature.RData")
```
...
```{r final_train}
tm_train=NA
#load("output/finalfeature.RData")
...
```{r superresolution}
source("lib/finalsuperresolution2.R")
test_dir <- "data/test_set2/" # This will be modified for different data sets.
...
HR_dir = 'data/train_set/HR/'
train_HR_dir <- paste(HR_dir, sep="")
HR_dir_test = 'data/test_set2/HR/'
test_HR_dir <- paste(HR_dir_test, sep="")
source("lib/psnr.R")
psnr = calculate_psnr(test_HR_dir, train_HR_dir)
mean(psnr)
```
