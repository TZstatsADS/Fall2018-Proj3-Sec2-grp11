...
superResolution <- function(LR_dir, HR_dir, modelList){
  ...
  for(i in 1:n_files){
    ...
    for (chann in 1:3){
      pad=cbind(0,imgLR[,,chann],0)
      pad=rbind(0,pad,0)
     
      featMat[,1,chann]=pad[cbind(select_row,select_col)]
      featMat[,2,chann]=pad[cbind(select_row,select_col+1)]
      featMat[,3,chann]=pad[cbind(select_row,select_col+2)]
      featMat[,4,chann]=pad[cbind(select_row+1,select_col+2)]
      featMat[,5,chann]=pad[cbind(select_row+2,select_col+2)]
      featMat[,6,chann]=pad[cbind(select_row+2,select_col+1)]
      featMat[,7,chann]=pad[cbind(select_row+2,select_col)]
      featMat[,8,chann]=pad[cbind(select_row+1,select_col)]
    }
